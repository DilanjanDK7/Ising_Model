...
def process_files(fmri_path, pet_path, jij_path,subject_id,parcellation,base_output_folder,global_results=None):
    ...
    if combine_fc:
        ...
        Jij_new= (a*Jij + b*empirical_fc_2)/(a+b)
    else:
        Jij_new = Jij
...
@njit(parallel=True)
def metropolis_step_all_spins(spin_array, beta, Jij=None, mu=None, alpha=1.0, J_default=1.0,
                              order='sequential', flip_mode='immediate', interaction_mode='mixed', h=1, seed=42):
    np.random.seed(seed)  # Set the seed for reproducibility
    N = spin_array.shape[0]  # Total number of spins

    total_steps = N * 4 if interaction_mode == 'mixed' else N
    for simulation_step in range(h):
        if order == 'sequential':
            indices = np.arange(total_steps)
        elif order == 'strength' and Jij is not None:
            interaction_strength = np.sum(np.abs(Jij), axis=1)
            indices = np.argsort(-interaction_strength)[:total_steps]
        else:
            indices = np.random.permutation(total_steps)

        flip_decisions = np.zeros(N, dtype=np.bool_)  # Size should be N to cover all spins

        for i in prange(total_steps):
            idx = indices[i % N]
            local_T = (mu[idx] ** alpha) / beta if mu is not None else 1.0 / beta
            local_beta = 1.0 / local_T

            # Decide which interaction function to use based on the mode
            if interaction_mode == 'mixed' and i < N:
                delta_E = delta_E_nearest_neighbors(idx, spin_array, J_default, N)
            elif interaction_mode == 'mixed' or interaction_mode == 'jij':
                delta_E = delta_E_arbitrary(idx, spin_array, Jij if Jij is not None else np.full((N, N), J_default), N)
            else:
                delta_E = delta_E_nearest_neighbors(idx, spin_array, J_default, N)
        ...
        if flip_mode == 'after_check':
            for i in prange(N):
                if flip_decisions[indices[i % N]]:
                    spin_array[indices[i % N]] *= -1
...
def optimize_and_simulate(time_series_path, N, steps_eq, steps_mc, output_folder, Jij=None,mu=None):
    ...
    # Step 1: Optimization
    optimized_params = optimize_parameters(time_series_path, N, steps_eq, steps_mc, Jij, bounds=((0.0001, 1), (-3, 3)),mu=mu,output_folder=output_folder)
