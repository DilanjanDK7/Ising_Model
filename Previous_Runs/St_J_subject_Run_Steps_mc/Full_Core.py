...
def remove_diagonal(matrix):
    ...
    return matrix
def discrepancy_function(params, empirical_fc, N, steps_eq, Jij=None, mu=None):
    """Calculate discrepancy between empirical and simulated FC."""
    temperature, alpha, steps_mc = params  # Unpack steps_mc from params
    steps_mc = int(steps_mc)  # Ensure steps_mc is an integer
    # Simulate time series with given parameters
    print("Temperature : ", temperature, " , alpha :", alpha, " , steps_mc : ", steps_mc)
    if np.isnan(temperature) or np.isnan(alpha):
        return 1000  # Handling NaNs
    ...
    distance = combined_matrix_distance(empirical_fc_no_diag, simulated_fc_no_diag, alpha=1, beta=0)
    return distance
...
def optimize_parameters(time_series_path, N, steps_eq, Jij=None, bounds=((0.01, 10), (-3, 3), (10, 5000)), mu=None, output_folder=None):
    empirical_fc = extract_rho(time_series_path)
    result = dual_annealing(discrepancy_function, bounds, args=(empirical_fc, N, steps_eq, Jij, mu),
                            maxfun=1e3)
    temp_optimized, alpha_optimized, steps_mc_optimized = result.x  # Unpack optimized steps_mc
    steps_mc_optimized = int(steps_mc_optimized)  # Ensure steps_mc is an integer
    return temp_optimized, alpha_optimized, steps_mc_optimized
...
def optimize_and_simulate(time_series_path, N, steps_eq, steps_mc, output_folder, Jij=None,mu=None):
    ...
    # Step 1: Optimization
    optimized_params = optimize_parameters(time_series_path, N, steps_eq,Jij, bounds=((0.01, 10), (-3, 3), (10, 5000)),mu=mu,output_folder=output_folder)
    temp_optimized, alpha_optimized,steps_mc_optimized = optimized_params
