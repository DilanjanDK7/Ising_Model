...
def metropolis_step_all_spins(spin_array, beta, Jij=None, mu=None, alpha=1.0, J_default=1.0, order='strength', flip_mode='immediate'):
    if Jij is not None:
    ...
    # Function to calculate energy change for arbitrary interactions
    def delta_E_arbitrary(i):
        return 2 * sum(Jij[i, j] * spin_array[i] * spin_array[j] for j in range(N) if i != j)

    # Ordering of indices based on the chosen strategy
    if Jij is not None:
        if order == 'sequential':
            indices = range(N)
        elif order == 'strength':
    ...
    for i in indices:
        ...
        # Decide whether to flip the spin immediately or after check
    ...
    # Apply the flip decisions if in 'after_check' mode
