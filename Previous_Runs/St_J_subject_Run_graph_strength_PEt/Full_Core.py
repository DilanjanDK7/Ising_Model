...
def process_files(fmri_path, pet_path, jij_path,subject_id,parcellation,base_output_folder):
    ...
    description_array = ['Optimal_Temperature', 'Optimal_Alpha','Optimal_Gamma','Parcellation', 'Subject','correlations','steps_eq','steps_mc']
    value_array = [optimised[0][0], optimised[0][1], optimised[0][2],parcellation, subject_id,optimised[1],optimised[5],optimised[6]]
...
def simulation_task(params):
    N, beta, steps_eq, steps_mc, Jij, mu, alpha, gamma, collect_time_series = params
    ...
    for _ in range(steps_mc):
        ...
        for _ in range(steps_eq):
            ...
            spin_matrix = metropolis_step_all_spins(spin_matrix, beta, Jij, mu, alpha, gamma)
            # This be Equilibrium step is addeed such that it ensures that in between time points the system is allowed to run 10
            # times to ensure that the system is in equilibrium in between every time step
        spin_matrix = metropolis_step_all_spins(spin_matrix, beta, Jij, mu, alpha, gamma)
...
#     time_series = []
#     for _ in range(steps_eq + steps_mc):
#         spin_array = metropolis_step_all_spins(spin_array, beta, Jij, mu, alpha, gamma)
#         if _ >= steps_eq:  # Collect after equilibration
#             time_series.append(spin_array.copy())
...
def discrepancy_function(params, empirical_fc, N, steps_eq, steps_mc, Jij=None, mu=None,global_results=None):
    ...
    if np.isnan(temperature) or np.isnan(alpha) or np.isnan(gamma):
    ...
    print("Temperature : ", temperature, " , alpha :", alpha, " , gamma : ", gamma)
    mag_mean, energy_mean, susceptibility, specific_heat,simulated_time_series = simulation_task((N, beta, steps_eq, steps_mc, Jij, mu, alpha, gamma, True))
...
def optimize_parameters(time_series_path, N, steps_eq, steps_mc, Jij=None, bounds=[(0.001,3), (-3, 3), (-3, 3)], mu=None,
                        output_folder=None):
    ...
    plt.close()
    np.save(os.path.join(output_folder,'Jij_optimized.npy'), Jij)
    ...
    result = dual_annealing(
        ...
        initial_temp=3000,  # Adjust the initial temperature if needed
...
def calculate_matrix_correlations_and_norms_upper_triangle(empirical_fc, simulated_fc, jij):
    """
    Calculates and returns the correlations and Frobenius norms between the upper triangles (excluding the diagonal)
    of empirical FC and simulated FC, empirical FC and Jij, and simulated FC and Jij.
    ...
    Returns:
    - A dictionary containing the correlation coefficients and Frobenius norms between the upper triangles of:
      empirical and simulated FC, empirical FC and Jij, and simulated FC and Jij.
    """

    # Extract the upper triangle of the matrices, excluding the diagonal
    empirical_fc_upper = empirical_fc[np.triu_indices_from(empirical_fc, k=1)]
    simulated_fc_upper = simulated_fc[np.triu_indices_from(simulated_fc, k=1)]
    jij_upper = jij[np.triu_indices_from(jij, k=1)]
    ...
    # For Frobenius norms between matrices, we first need to create masks of the upper triangle to apply before subtraction
    upper_triangle_mask = np.triu(np.ones(empirical_fc.shape), k=1)
    fro_norm_empirical_simulated = np.linalg.norm((empirical_fc - simulated_fc) * upper_triangle_mask, 'fro')
    fro_norm_empirical_jij = np.linalg.norm((empirical_fc - jij) * upper_triangle_mask, 'fro')
    fro_norm_simulated_jij = np.linalg.norm((simulated_fc - jij) * upper_triangle_mask, 'fro')
...
def optimize_and_simulate(time_series_path, N, steps_eq, steps_mc, output_folder, Jij=None,mu=None):
    ...
    # Step 1: Optimization
    optimized_params = optimize_parameters(time_series_path, N, steps_eq, steps_mc, Jij, bounds=[(0.001, 3), (-3, 3), (-3, 3)],mu=mu,output_folder=output_folder)
    temp_optimized, alpha_optimized,gamma_optimized = optimized_params
    ...
    _, _, _, _, time_series_optimized = simulation_task(
        (N, beta_optimized, steps_eq, steps_mc, Jij, None, alpha_optimized,gamma_optimized,True))
    ...
    if Jij is not None:
        ...
        results = calculate_matrix_correlations_and_norms_upper_triangle(empirical_fc_no_diag, simulated_fc_no_diag, Jij)
    ...
    return optimized_params,results,os.path.join(output_folder, 'fc_matrix_optimized.npy'), os.path.join(output_folder,
                                                                                                  'time_series_optimized.npy'), log_path, steps_eq , steps_mc
