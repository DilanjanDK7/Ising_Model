...
def remove_diagonal(matrix):
    ...
    return matrix
def discrepancy_function(params, empirical_fc, N, steps_eq, steps_mc, Jij=None, mu=None,output_folder=None):
...
def optimize_parameters(time_series_path, N, steps_eq, steps_mc, Jij=None, bounds=((0.001, 3), (-3, 3)), mu=None,
                        output_folder=None):
    ...
    # Visualizing the correlation matrix
    plt.figure(figsize=(10, 10))
    sns.heatmap(empirical_fc_no_diag, annot=True, fmt=".2f", cmap='coolwarm')
    plt.title("Functional Connectivity Matrix")
    ...
    result = dual_annealing(discrepancy_function, bounds, args=(empirical_fc_no_diag, N, steps_eq, steps_mc, Jij, mu,output_folder),
                            maxfun=1e2)
