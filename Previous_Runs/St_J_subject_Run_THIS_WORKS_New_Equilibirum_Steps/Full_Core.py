...
def process_files(fmri_path, pet_path, jij_path,subject_id,parcellation,base_output_folder):
    ...
    description_array = ['Optimal_Temperature', 'Optimal_Alpha', 'Parcellation', 'Subject','correlations','steps_eq','steps_mc']
    value_array = [optimised[0][0], optimised[0][1], parcellation, subject_id,optimised[1],optimised[5],optimised[6]]
...
def metropolis_step_all_spins(spin_array, beta, Jij=None, mu=None, alpha=1.0, J_default=1.0):
    ...
    for _ in range(N):  # Attempt to update each spin once on average per sweep
        i = np.random.randint(N)  # Select a random spin index
...
def optimize_parameters(time_series_path, N, steps_eq, steps_mc, Jij=None, bounds=((0.01, 10), (-3, 3)), mu=None,
    ...
    result = dual_annealing(
        ...
        visit=2,  # Increase the visit parameter
