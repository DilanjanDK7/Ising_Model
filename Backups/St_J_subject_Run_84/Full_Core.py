...
def optimize_parameters(time_series_path, N, steps_eq, steps_mc, Jij=None, bounds=((0.01, 10), (-3, 3)), mu=None,
    ...
    # Visualizing the correlation matrix
    plt.figure(figsize=(10, 10))
    sns.heatmap(empirical_fc, annot=False, fmt=".2f", cmap='coolwarm')
    plt.title("Functional Connectivity Matrix")
    ...
    result = dual_annealing(discrepancy_function, bounds, args=(empirical_fc, N, steps_eq, steps_mc, Jij, mu),
                            maxfun=1e2)
...
def optimize_and_simulate(time_series_path, N, steps_eq, steps_mc, output_folder, Jij=None,mu=None):
    ...
    # Step 1: Optimization
    optimized_params = optimize_parameters(time_series_path, N, steps_eq, steps_mc, Jij, bounds=((0.01, 20), (-5, 5)),mu=mu,output_folder=output_folder)
    ...
    plt.close()
    plt.figure(figsize=(10, 10))
    sns.heatmap(fc_matrix_optimized, annot=False, fmt=".2f", cmap='coolwarm')
