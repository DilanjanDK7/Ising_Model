...
def simulation_task_modified(params):
    ...
    for _ in range(steps_mc):
        spin_matrix = metropolis_step_all_spins(spin_matrix, beta, Jij, mu, alpha)
        mag = np.abs(np.sum(spin_matrix)) / N
        mags.append(mag)
    mag_mean = np.mean(mags)
    susceptibility = (np.var(mags) * N) * beta
    return mag_mean, susceptibility

def run_simulation_over_temps(N, temp_range, steps_eq, steps_mc, Jij=None, mu=None):
    results = {'temperature': [], 'magnetization': [], 'susceptibility': []}
    for temp in temp_range:
        print(f"Running simulation for temperature: {temp:.2f}")
        beta = 1.0 / temp
        mag_mean, susceptibility = simulation_task_modified((N, beta, steps_eq, steps_mc, Jij, mu, 1.0, False))
        results['temperature'].append(temp)
        results['magnetization'].append(mag_mean)
        results['susceptibility'].append(susceptibility)
    return results

def test_metropolis_and_find_tc(N, temp_start, temp_end, steps_eq, steps_mc, Jij=None, mu=None):
    temp_range = np.linspace(temp_start, temp_end, 50)  # Adjust as needed
    results = run_simulation_over_temps(N, temp_range, steps_eq, steps_mc, Jij, mu)

    # Find the peak in susceptibility
    susceptibilities = np.array(results['susceptibility'])
    tc_index = np.argmax(susceptibilities)
    estimated_tc = results['temperature'][tc_index]

    # Plot results for visualization
    plt.figure(figsize=(10, 4))
    plt.subplot(1, 2, 1)
    plt.plot(results['temperature'], results['magnetization'], label='Magnetization')
    plt.axvline(x=estimated_tc, color='r', linestyle='--', label=f'Estimated $T_c$ = {estimated_tc:.2f}')
    plt.xlabel('Temperature')
    plt.ylabel('Magnetization')
    plt.legend()

    plt.subplot(1, 2, 2)
    plt.plot(results['temperature'], results['susceptibility'], label='Susceptibility')
    plt.axvline(x=estimated_tc, color='r', linestyle='--', label=f'Estimated $T_c$ = {estimated_tc:.2f}')
    plt.xlabel('Temperature')
    ...
    # Check if the function worked correctly by comparing to the known value
    known_tc = 2.269
    print(f"Estimated Tc: {estimated_tc:.4f}, Known Tc: {known_tc}")
    if np.abs(estimated_tc - known_tc) <= 0.1:  # Allowable error margin
        print("Function worked correctly: Estimated Tc is close to the known value.")
    else:
        print("Function may not have worked correctly: Estimated Tc is far from the known value.")
...
#     # Optimize temperature and alpha
#     result = dual_annealing(discrepancy_function, bounds, args=(empirical_fc_no_diag, N, steps_eq, steps_mc, Jij, mu,global_results),
#                             maxfun=1e2)
#
#
#
...
def optimize_parameters(time_series_path, N, steps_eq, steps_mc, Jij=None, bounds=((0.01, 10), (-3, 3)), mu=None,
    ...
    result = dual_annealing(discrepancy_function, bounds,
    ...
    peaks, _ = find_peaks(susceptibilities)
    Tc_estimated = temperatures[peaks[0]] if peaks.size > 0 else None
...
def plot_observables_vs_temperature(global_results, Tc, optimal_temperature, output_folder):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(global_results['temperature'], global_results['magnetization'], label='Magnetization')
    ...
    plt.subplot(1, 2, 2)
    plt.plot(global_results['temperature'], global_results['susceptibility'], label='Susceptibility')
    plt.axvline(x=Tc, color='r', linestyle='--', label=f'Estimated Tc={Tc}')
    plt.axvline(x=optimal_temperature, color='g', linestyle='--', label=f'Optimal Temperature={optimal_temperature}')
    plt.xlabel('Temperature')
    plt.ylabel('Susceptibility')
